...
/** Material Symbols "directions_run" (Apache-2.0), same glyph as the tab bar so
 *  a run reads identically wherever it appears. Filled, not stroked. */
export function RunGlyph({ size = 14, className }: { size?: number; className?: string }) {
  // className wins over size so a caller can scale the glyph off its container
  return (
    <svg
...
/** Small dumbbell, matching the Train tab icon. */
export function DumbbellGlyph({ size = 14 }: { size?: number }) {
  return (
